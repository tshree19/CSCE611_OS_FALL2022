...
/* -- COMMENT/UNCOMMENT THE FOLLOWING LINE TO EXCLUDE/INCLUDE THE ROUND ROBIN SCHEDULER CODE*/

#define _RR_SCHEDULER_
/* This macro is defined when we want to force the code below to use the Round Robin Scheduler.
   Otherwise, either no scheduler is used , or FIFO scheduler is used and the threads pass control
   to each other in co-routine fashion.*/
...
void fun1() {
    ...
    for(int j = 0; j < 10; j++) 
#else
    for(int j = 0; j < DEFAULT_NB_ITER; j++) 
#endif
    {	
...
int main() {
    ...
    RRScheduler *SYSTEM_SCHEDULER = new RRScheduler(5);
#else
    //if FIFO scheduler, then simple timer implemented.
    SimpleTimer timer(100);
    InterruptHandler::register_handler(0, &timer);/* The Timer is implemented as an interrupt handler. */
