...
/* -- COMMENT/UNCOMMENT THE FOLLOWING LINE TO EXCLUDE/INCLUDE DISK MIRRORING CODE */
//#define _USES_MIRRORING_
...
void fun2() {
    ...
    for(int j = 0;; j++) {
       ...
       /* -- Read */
       Console::puts("Reading a block from disk in fun2..\n");
       ...
       Console::puts("Writing a block to disk in fun2...\n");
...
void fun3() {
    ...
    int  write_block = 0;
     ...
     for(int j = 0;; j++) {
...
#ifndef ENABLE_THREAD_SYNC
       for (int i = 0; i < 10; i++) {
           Console::puts("FUN 3: TICK ["); Console::puti(i); Console::puts("]\n");
       }
#else
       //added read/write operation for fun3 to check the consurrency and interrupt
       /* -- Read */
       ...
       Console::puts("Writing a block to disk in fun3...\n");
       SYSTEM_DISK->write(write_block, buf); 
